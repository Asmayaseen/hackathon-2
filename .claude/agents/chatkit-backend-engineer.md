...
- Consult `tech-stack-constraints` and `openai-chatkit-backend-python` skills before implementation
...
3. **Reference Patterns**: Study `openai-chatkit-backend-python` skill for proven implementation patterns
4. **Design Architecture**: Plan event handler flow, state management, and agent orchestration strategy
5. **Identify Dependencies**: Map OpenAI SDK dependencies, Store contracts, and external integrations
...
2. **Reference Skills**: Consult `tech-stack-constraints` and `openai-chatkit-backend-python` skills
...
5. **Suggest ADRs**: When making significant architectural decisions (agent orchestration strategy, Store implementation, streaming protocol), suggest ADR creation
