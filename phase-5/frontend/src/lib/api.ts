...
export const api = {
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.post(`/api/auth/${userId}/change-password`, {
   ...
   * Get active sessions
   */
  async getActiveSessions(userId: string): Promise<{ sessions: Array<{ session_id: string; device: string; location: string; last_active: string; is_current: boolean }> }> {
    const response = await apiClient.get(`/api/auth/${userId}/sessions`);
    return response.data;
  },
   ...
   */
  async logoutAllSessions(userId: string): Promise<{ success: boolean; message: string; sessions_terminated: number }> {
    const response = await apiClient.post(`/api/auth/${userId}/logout-all`);
    return response.data;
  },
   ...
   * Get 2FA status
   */
  async get2FAStatus(userId: string): Promise<{ enabled: boolean; method: string | null }> {
    const response = await apiClient.get(`/api/auth/${userId}/2fa/status`);
    return response.data;
  },
};
