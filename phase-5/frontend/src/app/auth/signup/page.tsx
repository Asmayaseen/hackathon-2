...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err.response?.data?.detail || t('auth.registrationFailed');
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden transition-colors duration-300">
      ...
      >
        ...
        <span className="hidden sm:inline text-sm font-medium uppercase tracking-wider">{t('common.back')}</span>
      </Link>
            ...
            <div className="relative bg-card/90 backdrop-blur-xl p-6 sm:p-8 rounded-2xl border-2 border-fuchsia-500/30">
              ...
              <div className="text-center mb-6 sm:mb-8">
                ...
                <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-fuchsia-400 to-cyan-400 bg-clip-text text-transparent mb-2">
                  {t('auth.joinTheNetwork')}
                </h1>
                <p className="text-muted-foreground text-sm uppercase tracking-wider">{t('auth.initializeNewAccount')}</p>
              </div>
