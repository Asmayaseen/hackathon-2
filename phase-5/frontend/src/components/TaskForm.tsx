...
export default function TaskForm({ userId, onTaskAdded }: TaskFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err.response?.data?.detail || t('tasks.failedToCreate');
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={handleSubmit} className="relative bg-card/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl mb-6 border-2 border-cyan-500/20">
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          placeholder={t('tasks.initializeNewTask')}
      ...
      {showDescription && (
        <div className="mt-3">
          <textarea
            ...
            placeholder={t('tasks.addMissionDetails')}
